	...
	<div class="logo">
		...
		<div class="row display-window-wrapper">
			<form name="app-app-form"
				action="${pageContext.request.contextPath}/AppUploadServlet"
				...
				<div class="app-category">
					<span class="name__info-text">App Category :</span> <select name="category">
					<%
						Category[] allCategories = (Category[])request.getAttribute("allCategories");
						for(int i=0; i<allCategories.length; i++) {
					%>
						<option value="<%= allCategories[i].getCategoryID() %>"><%= allCategories[i].getCategoryName() %></option>
					<%} %>
					</select>
				</div>
