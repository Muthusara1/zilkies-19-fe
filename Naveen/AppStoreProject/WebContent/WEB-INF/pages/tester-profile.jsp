...
<link rel="stylesheet"
	href="${pageContext.request.contextPath}/stylesheets/tester-profile.css">
<link rel="stylesheet"
...
<body>
	<div class="outer-container">
		...
		<div class="row display-window-wrapper">
			...
			<%
				Applications[] unpublishedApps = (Applications[])request.getAttribute("unpublishedApps");
			...
			<div class="pending-apps__app">
				<div class="app__dp">
					<img src="${pageContext.request.contextPath}/GetImages?id=<%= unpublishedApps[i].getAppID() %>"
						alt="tomb raider" class="dp__picture">
				</div>
				<div class="app__info">
					<div class="info__name"><%= unpublishedApps[i].getAppName() %></div>
					<div class="info__name">Category: <%= unpublishedApps[i].getCategory().getCategoryName() %></div>
					...
					<div class="info__options">
						<div class="options__remove">
							<button class="remove__button" onclick="removeApp(<%= unpublishedApps[i].getAppID()%>)">Remove</button>
						</div>
						<div class="options__get-it">
							<button class="get-it__button"
								onclick="testerVerified(<%= unpublishedApps[i].getAppID()%>)">Accept</button>
						</div>
					</div>
				</div>
			</div>
			<%} } else {
				out.print("No apps pending");
			}%>
		</div>
	</div>
	<script
		src="${pageContext.request.contextPath}/scripts/tester-profile.js"></script>
</body>
