...
public class AppsDao {
	...
	public void removeFromAllDownloads(Applications app) throws Exception {
		LOGGER.info("Entered removeFromAllDownloads");
		try {
			conn = connections.open();
			query = conn.prepareStatement(DatabaseQueries.REMOVE_ALL_DOWNLOADS);
			query.setInt(1, app.getAppID());
			int status = query.executeUpdate();
			if (status < 0) {
				throw new SystemException(Errors.DATABASE_ERR);
			...
			connections.close(conn);
			LOGGER.info("Exited removeFromAllDownloads");
		}
	}
	
	public void removeFromAllWishList(Applications app) throws Exception {
		LOGGER.info("Entered removeFromAllWishList");
		try {
			conn = connections.open();
			query = conn.prepareStatement(DatabaseQueries.REMOVE_ALL_WISHLIST);
			query.setInt(1, app.getAppID());
			int status = query.executeUpdate();
			if (status < 0) {
				throw new SystemException(Errors.DATABASE_ERR);
			...
			connections.close(conn);
			LOGGER.info("Exited removeFromAllWishList");
		}
	}
	...
	public Category[] getAllCategories() throws Exception {
		LOGGER.info("Entered getAllCategories");
		Category[] allCategories = null;
		try {
			conn = connections.open();
			query = conn.prepareStatement(DatabaseQueries.GET_ALL_CATEGORY);
			result = query.executeQuery();
			result.last();
			allCategories = new Category[result.getRow()];
			result.beforeFirst();
			int index = 0;
			while(result.next()) {
				allCategories[index] = new Category();
				allCategories[index].setCategoryID(result.getInt("CategoryID"));
				allCategories[index].setCategoryName(result.getString("CategoryName"));
				index++;
			}
		} catch (Exception e) {
			LOGGER.info(e.getMessage());
			...
			connections.close(conn);
			LOGGER.info("Exited getAllCategories");
		}
