...
@WebServlet("/DeleteAppFromDownloads")
public class DeleteAppFromDownloads extends HttpServlet {
	private static final long serialVersionUID = 1L;
    static Logger LOGGER;
	
    public DeleteAppFromDownloads() {
        super();
        LOGGER = Logger.getLogger(DeleteAppFromDownloads.class.getName());
    }
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if (request.getSession(false).getAttribute("user") == null) {
				...
				appController.deleteAppFromDownloads(app, user);
