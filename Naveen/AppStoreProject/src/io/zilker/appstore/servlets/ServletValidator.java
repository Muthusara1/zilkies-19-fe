...
public class ServletValidator {
	...
	public void checkCategory(String id) throws Exception {
		LOGGER.info("Entered checkCategory");
		try {
			if(Integer.parseInt(id) < 1 || Integer.parseInt(id) > 6)
				throw new BusinessException(Errors.WRONG_CATEGORY);
		} catch(Exception e) {
			LOGGER.info(e.getMessage());
			throw new BusinessException(e.getMessage());
		} finally {
			LOGGER.info("Exited checkCategory");
		}
	}
